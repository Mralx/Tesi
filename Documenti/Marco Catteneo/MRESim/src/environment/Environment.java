...
// <editor-fold defaultstate="collapsed" desc="Information of Interest">
    ...
    public int getTotalObsSpace() {
        int runningTotal = 0;

        for(int i=0; i<rows; i++){
            for(int j=getBorderIdx(i, true); j<getBorderIdx(i,false); j++){
                if(status[j][i]==Status.obstacle )
                            runningTotal++;
                }
            }
    ...
    private int getBorderIdx(int rowIdx, boolean start){
        if(start){
            for(int j=0; j<columns; j++){
                if(status[j][rowIdx]!=Status.obstacle)
                    return j;
            }
            return 0;
        }
